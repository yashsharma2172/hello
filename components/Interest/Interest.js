...
import { MENULINKS, INTEREST } from "../../constants";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/dist/ScrollTrigger";
import ProjectTile from './InterestTile/InterestTile'

const Interest = ({ isDesktop, clientHeight }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col py- justify-center h-full">
        ...
        >
          {INTEREST.map((project, index) => (
            <ProjectTile
              classes={
                index === INTEREST.length - 1 ? "" : "mr-10 xs:mr-12 sm:mr-16"
              }
              project={project}
              key={project.name}
            />
